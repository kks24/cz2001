...
public class Graph2 {
	...
	public Graph2(int numOfVertices,int noEdges) {
		...
		if(noEdges>0) {
			for (int i = 0 ; i < numOfVertices; i ++) {
				int v2 = (int) (Math.random()*(numOfVertices - 1));
				boolean existing = adj[i].edges.contains(v2);
				if (v2 != i & !existing) {
					insertEdges(i,v2);
				}
				else {
			...
			// getting data 
			for(int i = numOfVertices;i<noEdges;i++) {
				int randomIndex1 = (int) (Math.random()*(numOfVertices - 1));
				int randomIndex2 = (int) (Math.random()*(numOfVertices - 1));
				int v1 = findNodeIndex(clist[randomIndex1]);
				int v2 = findNodeIndex(clist[randomIndex2]);
				boolean existing = adj[v1].edges.contains(v2);
				if (v1 != -1 & v2 != -1 & v1 != v2 & !existing) {
					insertEdges(v1,v2);
				}
				else {
					i --;
	...
	// BFS for testing
	public void BFS(String sourceName, String destinationName, boolean testing) {        
		// Reset all data in node
		for (int count = 0; count < numOfVertices; count ++) {
			adj[count].nodeLength = 0;
			adj[count].previousIndex = -1;
